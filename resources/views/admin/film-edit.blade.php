...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">       
            ...
            <form class="form-new-product form-style-1" action="{{route('admin.film.update')}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="id" value="{{$film->id}}" />
                <fieldset class="name">
                    <div class="body-title">Film Name <span class="tf-color-1">*</span></div>
                    <input class="flex-grow" type="text" placeholder="Film Name" name="name" value="{{ $film->name }}">
                </fieldset>
                ...
                <fieldset class="genre">
                    ...
                    <select name="genres[]" multiple many-relation>
                        @foreach($genres as $genre)
                            <option value="{{$genre->id}}" {{$film->genres->contains('id',$genre->id) ? "selected":""}}>{{$genre->name}}</option>
                        @endforeach                                                 
                    </select>                        
                </fieldset>
